...
function parseText(str){
  ...
  if (containsWaveArm) {
   ...
   waveArm("wave") ;
   play_or_tts_ch("哈嘍，你好，我是tj,robot，很高興認識你，來跟我聊聊天吧！");
  } /* else if (cute){
   waveArm("wave") ;
...
function play_or_tts_ch(data,callback){
	...
	if(tw==0){
	console.log(hash);
	var wav_file = '/home/pi/voice/tts_ch_6/' + hash + ".wav";
			if(fs.existsSync(wav_file))
			{
			 ...
			 console.log('execute iflytec tts');
		     var mic= exec('LD_LIBRARY_PATH=/home/pi/workspace /home/pi/workspace/tts_sample '+text+' '+wav_file, function(err,stdout, strerr){
				 console.log(stdout);
				 console.log(err);
        if(!err){
...
function play_and_block_mic(wav_file,callback){
	...
	//		if(speaker_using==false){
				...
				speaker_using=true;
				//iot_client.publish('iot-2/evt/speak_use/fmt/string','true');
				micInstance.pause(); // pause the microphone while playing
				//exec("amixer -c 0 sset Mic nocap");
				//gled.write(0);
				console.log('waiting....'); 
				//pigpio.terminate();
				processing_music.on('complete',function(){
					console.log("finish play");
					//exec("amixer -c 0 sset Mic cap");
					  micInstance.resume();
					  console.log('mic resume');
				    //speaker_using=false;
					//mic.stdout.pipe(recognizeStream);
					//openCMDS();
					//iot_client.publish('iot-2/evt/speak_use/fmt/string','false');
					speaker_using=false;
